...
namespace GameEngine.GameLogic
{
    public class CollisionLogic
    {
        ...
        public static void AsteroidsCollision(List<Asteroid> asteroids, List<Bullet> bullets, Ship ship, ref int score)
        {
            for (int i = 0; i < asteroids.Count; i++)
            {
                ...
                for (int j = 0; j < bullets.Count; j++)
                {
                    if (asteroids[i].Collision(bullets[j]))
                    {
                        ...
                        {
                            CollisionLogic.CreateLitleAsteroids(asteroid, asteroids);
                        }
                        score += 30;
                        if (i != 0)
                            i--;
        ...
        public static void CreateLitleAsteroids(Asteroid parentAsteroid, List<Asteroid> asteroids)
        {
